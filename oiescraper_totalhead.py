    ...
    def sum_groupby():
        ...
        for x in sum_list:
            if x == '구분':
                ...
                total = table_modify.groupby(['질병','국가','리포트 번호','축종'])[x].sum()
...
if __name__ == '__main__':
    ...
    # # 특정 행 이후
    # table = pd.read_sql("SELECT * FROM oie_reports_kr WHERE rowid BETWEEN 80650 AND 81000", con)  # rowid: 행

    # con.close()

    table = pd.read_excel("./oie_reports.xlsx", sheet_name='번역본')
