...
def first_groupby():
    total_first_list = []
    for x in first_list:
        total=table.groupby(['질병','국가','리포트 번호','축종'])[x].first()
        if total.name=='발생일':
            total.name='시작일'
        else: pass
...
def last_groupby():
    total_last_list = []
    for x in last_list:
        total=table.groupby(['질병','국가','리포트 번호','축종'])[x].last()
        if total.name=='발생일':
            total.name='종료일'
        else: pass
...
def sum_groupby():
    ...
    for x in sum_list:
        if x == '구분':
            total = table.groupby(['질병','국가','리포트 번호','축종'])[x].apply(lambda x: "%s"%','.join(x))
        elif x == '링크 번호':
...
table=pd.read_excel(r"C:\Users\kwonok59\Desktop\python project\fast report\oie_reports_200922.xlsx",sheet_name="번역본")
table['리포트 번호']=table['리포트 번호'].apply(immediate_followup)
